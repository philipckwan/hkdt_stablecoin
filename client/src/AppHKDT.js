import './AppHKDT.css';
import {timeLog, mapEthereumNetwork} from './PCKUtils'
import {useEffect, useState, useRef} from 'react'

import {USDC_ADDRESS, HKDT_ADDRESS, web3Sign, web3Verify, getNativeBalance, getERC20Balance, approveUSDCtoHKDT, depositUSDCToMintHKDT, burnHKDTToWithdrawUSDC} from "./EthersUtils";
...
function AppHKDT() {
  ...
  useEffect(() => {
    (async () => {
      const nativeBalanceFromUtils = await getNativeBalance(connectedWalletAddress);
      setNativeBalance(nativeBalanceFromUtils);
    })();
    return () => {
      ...
      <div>
        <table className="myTable">
          ...
          <tr>
            ...
            <td>{mapEthereumNetwork(window.ethereum.networkVersion)[0]}</td>
          </tr>     
          <tr>
